...
void main()
{
    ...
    vec4 temp = vec4(aPos, 1.0f);
    vec2 dir = normalize(vec2(1.0, 1.0));
    temp.x += dir.x * uAmplitude * cos(dot(dir, temp.xz) * uFrequency - uTime * uSpeed);
    temp.z += dir.y * uAmplitude * cos(dot(dir, temp.xz) * uFrequency - uTime * uSpeed);
    temp.y += uAmplitude * sin(dot(dir, temp.xz) * uFrequency - uTime * uSpeed);

    //vec4 temp = vec4(aPos, 1.0f);
   // temp.y += sin(aPos.x * uFrequency + uTime * uSpeed) * uAmplitude;
    //temp.y += cos(aPos.z * uFrequency + uTime * uSpeed) * uAmplitude;

    //Set vertex to world space
    vec4 worldPos = (modelMat * temp);
