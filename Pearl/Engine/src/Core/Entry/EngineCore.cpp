...
#include "Core/Entry/EngineCore.h"
...
namespace PrCore {
	...
	EngineCore::EngineCore(std::unique_ptr<IContext>&& p_context) :
		m_context(std::move(p_context)),
		m_physicsStepAccumulator(0.0f)
	{
	}

	bool EngineCore::Initalize(const EngineCoreParams& p_params)
	{
		if (m_context == nullptr)
		{
			PR_ASSERT(false, "Cannot create Engine Core without context!");
			return false;
		}

		// Init Engine Subsystems
		PrSystems::Register<PrCore::Utils::ILogger, PrCore::Utils::Logger>();
		PRLOG_INFO("Initalizing EngineCore");
		...
		if (contexConfig.OpenFromFile(GraphicConfig))
		{
			...
			PrSystems::Register<IWindow, GLWindow>(context, windowSettings);
		}
		...
		//-----------------------
		// Init Context
		if (!m_context->OnInitalize(p_params))
		{
			PRLOG_ERROR("Cannot initalize context!");
			return false;
		}
	...
	bool EngineCore::Run()
	{
		//-----------------------
		// PreFrame
		float deltaTime = PrSystems::Get<Utils::Clock>()->GetUnscaledDeltaTime();

		//-----------------------
		// Frame Update

		//-----------------------
		// Input and Window Update 
		PrSystems::Get<PrCore::InputManager>()->ResetFlags();
		PrSystems::Get<IWindow>()->PollEvents();

		//-----------------------
		// Context Update
		if (!m_context->OnUpdate(deltaTime))
			return false;
		...
		PrSystems::Get<IWindow>()->SwapBuffers();
	...
	{
		return std::make_unique<EngineCore>(std::move(p_context));
	}
}