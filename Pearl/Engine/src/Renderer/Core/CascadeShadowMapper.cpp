...
PrCore::Math::vec4 CascadeShadowUtility::CalculateShadowMapCoords(size_t p_index, size_t subMapSize, size_t mapSize) const
{
	size_t rowCount = mapSize / subMapSize;
	size_t row = p_index % rowCount;
	size_t column = p_index / rowCount;

	return { subMapSize, subMapSize, row * subMapSize, column * subMapSize };
}
