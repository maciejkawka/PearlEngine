...
std::shared_ptr<Texture2Dv2> Texture2Dv2::Create()
{
	Texture2Dv2Ptr texture = nullptr;
	switch (Core::RendererAPI::GetGraphicsAPI())
	{
	case Core::GraphicsAPI::OpenGL: texture = std::make_shared<OpenGL::GLTexture2Dv2>(); break;
...
std::shared_ptr<Texture2Dv2> Texture2Dv2::CreateUnitTex(const Core::Color& p_unitColor)
{
