...
namespace PrRenderer::Resources {
	...
	class Materialv2 : public PrCore::Resources::IResourceData {
	public:
		Materialv2() = delete;
		Materialv2(ShaderPtr p_shader);
		Materialv2(const Materialv2& p_material);
		Materialv2(PrCore::Utils::JSON::json& p_seralizedMat);
		...
		void CopyPropertiesFrom(const Materialv2& p_material);
	...
	template<typename T>
	inline void Materialv2::SetProperty(const std::string& p_name, const T& p_value)
	{
	...
	template<typename T>
	inline void Materialv2::SetPropertyArray(const std::string& p_name, const T const* p_value, unsigned int p_count)
	{
		auto find = m_uniforms.find(p_name);
		if (find != m_uniforms.end())
		{
	...
	template<typename T>
	inline const T& Materialv2::GetProperty(const std::string& p_name)
	{
